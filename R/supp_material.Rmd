...
Each function works with two types of input: vector and matrix. If the input is a matrix, the user must change the default argument *is_vector* to *FALSE*. The matrix is then transformed to independent vectors using Detrented Correspondance Analysis [@hill_detrended_1980], and a single axis on which changepoint analysis is run is chosen with the argument *DCA_axis* (default to 1, for first component). Component scores are returned by the funtion. User can edit the function to use another ordination method (e.g., principal component analysis).


# Method's sensitivity for varying parameters
...
The iterative method requires overall less subsamples to find the real changepoints (Fig. \ref{fig:case_study_other_changepoints}, 3rd column). Note that if the initial number of samples (controled with the argument *n2* in the code) is at the default 5, the code struggles to rank appropriately the changepoints, resulting in a changepoint being found, but not the overall "true" second  (e.g., Fig. \ref{fig:case_study_other_changepoints}b, 3rd column), or "true" third. In a time-series, we recommend users to examinethe p-values returned by the *e.divisive* function, embeded in the funtion *sample_iterative* output. If the first few changepoints have close, low, p-values, it may be worth to not directly pursue a subsample close to the first changepoint, but also add another subsample further away. An alternative is to initiate the subsampling with an higher number of samples, e.g., 7 subsamples.
...
# Case study: changepoint on a vector of number of individuals



```{r echo=FALSE, eval=TRUE, message=FALSE, warning=FALSE, fig.width = 4, fig.height= 3, fig.cap='Number of individuals per gram of sediment, infered from Cladocera remains for the 74 subsamples of the Lake Varese sediment core.   \\label{fig:case_study_biomass_per_year}'}
...
For further ilustration, we reproduced the approach on a single vector. We re-used the community dataset from Lake Varese, but this time, looked at the number of individuals per gram of sediment in each subsamples. The full time series show a changepoint in `r paste(vr_count$Year[cpvar_count$changepoint], vr_count$Year[cpvar_count$changepoint-1], sep="/")`. We used the three changepoint method to estimate changepoint on the vector, and found that the iterative method, with an initial number of 5 regularly-spaced subsamples, performed best (Fig. \ref{fig:case_study_number_sample_biomass}).
