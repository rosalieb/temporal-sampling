...
```
<br>`r fig_cap("percentage explained by PCs", "Percentage explained by PC axes according to number of samples analysed")`
...
```
<br>`r fig_cap("timing of cpts", "Timing of changepoint")`
...
```{r}
# Change here the sample size
sample_size <- c(rep(13:30, each=30))
#sample_size <- 15
output <- as.data.frame(matrix(rep(NA, 5*length(sample_size)), ncol=5))
...
```
<br>`r fig_cap("how many samples must be added to get true changepoint", "A 'biased' sampling (trying to detect the true shift by getting closest sample) is not a guarantee to reduce the total number of samples that should be analyzed to detect the true shift")`
...
```
<br>`r fig_cap("how many samples must be added to get true changepoint method 2", "Adding progressively samples to fill in the gaps between transition and next/previous sample is a better strategy.")`
...
# Chances of getting close to the real break

Following discussion with Easton on 10/10/2019.

```{r}
# Change here the threshold year: how close you need to get close to the actual year
threshold_yr = c(2,4,6,8,10,12,14)

# Change here the sample size
sample_size <- c(rep(15:50, each=100))
#sample_size <- 15
output3 <- as.data.frame(matrix(rep(NA, 4*length(sample_size)*length(threshold_yr)), ncol=4))
colnames(output3) <- c("Initial_number_samples","threshold_yr","Initial_AMOC_PC1","Within_threshold")
...
for (i in 1:length(sample_size)) {
  ...
  for (j in 1:length(threshold_yr)) {
    row=row+1
      # 1. Sample years
  which_samples <- sample(2:(nrow(zh)-1), size = sample_size[i],replace = F)
  which_samples <- c(which_samples,1,nrow(zh))
  ...
  cptmean.AMOC.PC1_num <- cptmean.AMOC.PC1
  cptmean.AMOC.PC1 <- as.numeric(mdata[,1][cpts(cptmean.AMOC.PC1)])
  if(length(cptmean.AMOC.PC1)==0) cptmean.AMOC.PC1 <- 0
  
  # Save outputs
  output3$Initial_number_samples[row]     <- sample_size[i]
  output3$threshold_yr[row]               <- threshold_yr[j]
  output3$Initial_AMOC_PC1[row]           <- cptmean.AMOC.PC1
  if(abs(cptmean.AMOC.PC1-true.cptmean.AMOC.PC1)<=threshold_yr[j]){
        numberPC1_ok = 1
        } else {numberPC1_ok = 0}
  
  output3$Within_threshold[row] <- numberPC1_ok
  }
...
outsumm <- as.data.frame(with(output3, tapply(Within_threshold,list("Initial_number_samples"=Initial_number_samples,"threshold_yr"=threshold_yr), sum)))
