...
Number of references: 35<!--\total{citenum}-->
...
Because ecological systems are dynamic in both space and time, it is essential that sampling designs account for spatio-temporal dynamics (Williams et al. 2018). To estimate a same parameter, population abundance, optimal spatial sampling strategies are based on spatially balanced sampling [@kermorvant_optimizing_2019], while optimal temporal sampling strategies are more cost efficient with a targeted sampling, e.g., around the period of reproduction [@jackson_sampling_2008]. In either case, sampling time and lcoations can be chosen in an iterative process to be cost-effective and reduce the uncertainty in the process (Hooten et al. 2009). With the ability to choose precisely when to sample, we can move beyond random, interval, or opportunistic sampling designs (citations). This is particuraly relevant in situations where a subset of samples already collected need to be analyzed. 
