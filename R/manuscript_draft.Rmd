...
Authors: \textsc{Rosalie Bruel$^{1}$ and Easton R. White$^{2}$}
\vspace{5 mm}
...
```{r theoretical_results_min_time, fig.cap='Theoretical results here detailing parameter sensitivity - we should make this about sample size instead of probability correct',message=F,warning=F,echo=F,eval=T}
...
par(mfrow=c(1,3))
# Sigma plot
sigma_vec = seq(0.1,20,1)
min_time=NULL

for (param_num in 1:length(sigma_vec)){
  min_time = c(min_time,calc_min_time('sigma',sigma_vec[param_num]))
  print(min_time)
}

plot(sigma_vec,min_time,ylab='Number of samples required',xlab='sigma',pch=16,cex.lab=1.3,las=1,ylim=c(0,25),col=1)
...
for (param_num in 1:length(phi_vec)){
  min_time = c(min_time,calc_min_time('phi',phi_vec[param_num]))
}

plot(phi_vec,min_time,ylab='Number of samples required',xlab='phi',pch=16,cex.lab=1.3,las=1,ylim=c(0,1),col=1)


# Shift size

shift_size_vec = seq(0.05,0.95,0.1)
min_time=NULL

for (param_num in 1:length(shift_size_vec)){
  min_time = c(min_time,calc_min_time('shift_size',shift_size_vec[param_num]))
}

plot(shift_size_vec,min_time,ylab='Number of samples required',xlab='shift size (percent)',pch=16,cex.lab=1.3,las=1,ylim=c(0,1),col=1)
